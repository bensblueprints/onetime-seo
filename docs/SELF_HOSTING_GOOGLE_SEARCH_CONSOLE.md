...
Connecting Google Search Console (GSC) lets OneTime SEO pull your real clicks,
impressions, positions, and URL inspection data, straight from Google.

It's **optional**: OneTime SEO runs fine without it, just without Search Console data.
...
2. Add an **Authorized redirect URI** that exactly matches your deployment's
   ...
   | ------------ | -------------------------------------------------------- |
   | Deployed     | `https://your-onetime-seo-domain.com/api/gsc/oauth/callback` |
...
Set these three values, then restart OneTime SEO:
...
Restart OneTime SEO so it picks up the new variables. For Docker, changing `.env`
...
- OneTime SEO uses your Google client to run the OAuth flow and stores the resulting
...
- Search Console data comes from your own Google account, so OneTime SEO never meters credits for it.
